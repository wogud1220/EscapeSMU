...
const Stage13_6 = () => {
  ...
  const route = useRoute<RouteProp<RootStackParamList, 'Stage13_6'>>();
  ...
  useEffect(() => {
    ...
    console.log('Stage13_6 log - College:', college);
  }, []);
  useEffect(() => {
  ...
  const handleOptionPress = async (value: number) => {
    ...
    if (value === 4) {
      try {
        await updateStageData(userId, actualCollege, 'Stage13_6_1');
      } catch (err) {
      ...
      Alert.alert('정답입니다!', '다음 스테이지로 이동합니다.', [
        {
          text: '확인',
          onPress: () =>
            navigation.navigate('Stage13_6_1', {
