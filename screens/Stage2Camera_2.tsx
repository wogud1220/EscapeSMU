...
const Stage2Camera_2 = ({navigation}: {navigation: any}) => {
  ...
  const takePicture = async () => {
    ...
    try {
      ...
      formData.append('stage', 'stage2_2');
      ...
      if (data.result === 'Pass') {
        try {
          await updateStageData(userId, actualCollege, 'Stage2_4');
        } catch (err) {
  ...
  const goToNextStage = async () => {
    ...
    await updateStageData(userId, actualCollege, 'Stage2_4');
    navigation.navigate('Stage2_4', {college: actualCollege, department});
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity onPress={goToNextStage} style={styles.tempButton}>
        <Text style={styles.buttonText}>Stage2_4로 이동</Text>
      </TouchableOpacity>
