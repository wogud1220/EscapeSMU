...
const Stage6_2 = () => {
  ...
  const startListening = async () => {
    if (!Voice || typeof Voice.start !== 'function') {
      console.error('❗ Voice 모듈이 초기화되지 않았습니다.');
      Alert.alert('Voice 오류', 'Voice 모듈이 정상적으로 불러와지지 않았습니다.');
      return;
    }
    ...
    try {
      ...
      console.error('🎤 음성 시작 실패:', e?.message || JSON.stringify(e, null, 2));
      Alert.alert('음성 인식 오류', 'Voice 모듈이 초기화되지 않았거나, 기기에서 음성 인식을 지원하지 않습니다.');
  ...
  const stopListening = async () => {
    ...
    try {
      ...
      if (recognizedText.trim() === '독도는 우리 땅') {
        await updateStageData(userId, actualCollege, 'Stage6_3');
...
const styles = StyleSheet.create({
  ...
  buttonText: {color: '#FFFFFF', fontSize: width * 0.045},
  backButton: {
    position: 'absolute',
    top: height * 0.05,
    left: width * 0.05,
  ...
  recognizedText: {marginTop: 10, color: '#222', fontWeight: '600'},
});
