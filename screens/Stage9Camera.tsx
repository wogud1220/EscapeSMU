...
type Stage9CameraRouteProp = RouteProp<RootStackParamList, 'Stage9Camera'>;

const SERVER_URL = 'http://34.47.88.216:8000/compare';

const Stage9Camera = ({navigation}: {navigation: any}) => {
  ...
  const route = useRoute<Stage9CameraRouteProp>();
  const {college, department} = route.params || {};
  const isFocused = useIsFocused(); // 화면 포커스 상태 가져옴
  useEffect(() => {
    if (permission && !device) {
      const timeout = setTimeout(() => {
        navigation.replace('Stage9_2', { college, department });
      }, 100);
  ...
  return (
    <View style={styles.container}>
      ...
      <Image
        source={require('../assets/designmoong2.png')}
      ...
      {/* <TouchableOpacity onPress={goToNextStage} style={styles.tempButton}>
        <Text style={styles.buttonText}>Stage9_2_1로 이동</Text>
      </TouchableOpacity> */}
