...
const options = [
  {label: '무도장', value: 1},
  {label: '스쿼시 코트', value: 2},
  {label: '스크린 골프장', value: 3},
  {label: '댄스 스튜디오', value: 4},
  {label: '당구장', value: 5},
];

const Stage11_5 = () => {
  const navigation = useNavigation<NavigationProp>();
  const route = useRoute<RouteProp<RootStackParamList, 'Stage11_5'>>();
  ...
  const handleOptionPress = async (value: number) => {
    ...
    if (value === 5) {
      await updateStageData(userId, college, 'Stage11_6'); // 스테이지 진행 정보 저장
      Alert.alert('정답입니다!', '다음 스테이지로 이동합니다.', [
        {
          text: '확인',
          onPress: () =>
            navigation.navigate('Stage11_6', {college, department}),
        },
      ]);
    } else {
      ...
      <ImageBackground
        ...
        <View style={styles.box}>
          <CustomText style={styles.text}>
            어느 정도 스포츠센터를 돌아봤니? 그러면 스포츠센터에 존재하지{' '}
          ...
          <CustomText style={styles.subText}>
            틀릴 시에는 다시 입력하기까지{' '}
            <CustomText style={styles.highlight}>5분</CustomText>을 기다려야해... 신중하자!
          </CustomText>
